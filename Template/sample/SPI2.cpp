...
extern "C" void TIM2_IRQHandler(void)
{
	...
	if (TIM_GetITStatus(TIM2, TIM_IT_Update) != RESET)
	{
		...
		uint8_t dat = SPI2_Transaction(*(uint8_t*)&duty);
		if (*(char*)&dat != prev_duty){
			error_flag = true;
		}
		//[data prev duty]
		itoa(str, *(char*)&dat);
